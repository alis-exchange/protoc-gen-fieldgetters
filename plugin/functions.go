...
import (
	...
	"github.com/alis-exchange/protoc-gen-fieldgetters/plugin/utils"
...
func (gr *Generator) generateFile(gen *protogen.Plugin, file *protogen.File) (*protogen.GeneratedFile, error) {
	...
	for _, msg := range file.Messages {
		err := generateMessageGetters(g, msg, gr.includeMsgMethods)
		if err != nil {
			return nil, err
		}
	}
...
func (sg *FieldsGetter) processFields(fields []*protogen.Field) error {
	...
	// Keep track of visited messages to prevent infinite recursion
	visitedMessages := map[string]bool{}
	...
	for _, field := range fields {
		if field.Desc.IsList() && sg.isList && utils.Contains(sg.kinds, field.Desc.Kind()) {
			...
			case protoreflect.EnumKind:
				{
					...
					sg.gen.P("}")
					sg.gen.P(fmt.Sprintf("return %s, nil", listVariableName))
					continue
				}
			...
			default:
				{
					sg.gen.P(fmt.Sprintf("return %s.Get%s(), nil", sg.msgVariableName, field.GoName))
					continue
				}
			}
		}
		...
		if field.Desc.Kind() == protoreflect.MessageKind && !utils.Contains(sg.kinds, protoreflect.MessageKind) {
			err := sg.processMessage([]*protogen.Field{field}, field.Message, visitedMessages)
			if err != nil {
				return err
			}
		}
		...
		returnStr := fmt.Sprintf("%s.Get%s()", sg.msgVariableName, field.GoName)
...
func (sg *FieldsGetter) processMessage(parentFields []*protogen.Field, message *protogen.Message, visited map[string]bool) error {
	...
	if visited[messageName] {
		return nil // Skip to prevent infinite recursion
	}
	...
	for _, field := range message.Fields {
		...
		if field.Desc.Kind() == protoreflect.MessageKind && !utils.Contains(sg.kinds, protoreflect.MessageKind) {
			err := sg.processMessage(append(parentFields, field), field.Message, visited)
			if err != nil {
				return err
			}
		...
		returnStr := fmt.Sprintf("%s%s.Get%s()", sg.msgVariableName, traverseStr, field.GoName)
		if field.Desc.Kind() == protoreflect.Int32Kind {
			returnStr = fmt.Sprintf("int64(%s)", returnStr)
		}
		if field.Desc.Kind() == protoreflect.FloatKind {
...
func (sg *FieldsGetter) fieldsLen(fields []*protogen.Field, visited map[string]bool) int {
	count := 0
	for _, field := range fields {
		if field.Desc.IsList() && sg.isList && utils.Contains(sg.kinds, field.Desc.Kind()) {
			count++
		...
		if field.Desc.Kind() == protoreflect.MessageKind {
			...
			// Check if we've already visited this message type
			if visited[messageName] {
				continue // Skip to prevent infinite recursion
			}

			// Mark the message as visited
			visited[messageName] = true

			count += sg.fieldsLen(field.Message.Fields, visited)
			continue
		}
	}
	return count
...
func generateMessageGetters(g *protogen.GeneratedFile, msg *protogen.Message, generateResourceMethods bool) error {
	msgVariableName := fmt.Sprintf("%sMsg", utils.ToLowerFirst(msg.GoIdent.GoName))
	...
	// StringGetter
	{
		...
		fieldsGetter := &FieldsGetter{
			gen:             g,
			msg:             msg,
			msgVariableName: msgVariableName,
		...
		if fieldsGetter.fieldsLen(msg.Fields, map[string]bool{}) > 0 {
			fieldsGetter.gen.P(fmt.Sprintf("%s := msg.(*%s)", msgVariableName, msg.GoIdent.GoName))
			fieldsGetter.gen.P()
		}
